 ...
 * Modified by JWNAM on 2022-02 for CLOB.
 * 
...
public class mCMT {
	static String			versionStr = "20220221";
...
*/
					...
					for (i = 1; i <= metaCount; i++) {
						...
						if (metaType.equals("CHAR") || metaType.equals("VARCHAR") || metaType.equals("VARCHAR2")
								...
								clobLineNo = 0;
								sb.append(" '"); // clob 에 값이 없는 경우 ''로 처리.
								while ((readLen = ic.read(clobData, 0, MAX_BUFFER_SIZE)) >= 0) { // 약 1.4M 읽음
									...
									while (idx < readLen) {
										...
										if ((idx < readLen) && (idx >= MAX_STRING_LENGTH) && ((idx % MAX_STRING_LENGTH) == 0))
											sb.append("'+\n '");
									}
								}
